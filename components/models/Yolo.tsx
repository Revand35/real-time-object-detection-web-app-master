...
} from '../../utils/triangle_similarity_distance';

const RES_TO_MODEL: [number[], string][] = [
  [[256, 256], 'yolov7-tiny_256x256.onnx'], // Use YOLOv7 as primary - more stable
...
const Yolo = (props: any) => {
  ...
  // This can be calibrated by user for better accuracy
  const [focalLength] = useState<number>(DEFAULT_FOCAL_LENGTH);
  ...
  useEffect(() => {
    const getSession = async () => {
      try {
        ...
        console.error('Model path attempted:', `/static/models/${modelName}`);
        console.error('Model name:', modelName);
        
        const errorMsg = error instanceof Error ? error.message : String(error);
        setLoadingError(`Failed to load ${modelName}: ${errorMsg}`);
        
        // Try loading a different YOLOv7 model - these are more stable
        const yolo7Models = RES_TO_MODEL.filter(([, name]) => name.includes('yolov7'));
        for (const [, altModelName] of yolo7Models) {
          if (altModelName !== modelName) {
            try {
              console.log(`Trying alternative model: ${altModelName}`);
              const altSession = await runModelUtils.createModelCpu(
                `/static/models/${altModelName}`
              );
              console.log('Alternative model loaded successfully:', altModelName);
              setModelName(altModelName);
              setModelResolution(RES_TO_MODEL.find(([, n]) => n === altModelName)![0]);
              setSession(altSession);
              setLoadingError(null);
              return;
            } catch (altError) {
              console.error(`Alternative model ${altModelName} also failed:`, altError);
            }
          }
        }
      }
  ...
  const postprocess = async (
    ...
    if (modelName in postprocessMap) {
      ...
      postprocessMap[modelName](ctx, modelResolution, tensor, conf2color, focalLength);
    }
  };
...
const postprocessYolov12: PostprocessFunction = (
  ...
  postprocessYolov11(ctx, modelResolution, tensor, conf2color, focalLength);
};
...
const postprocessYolov11: PostprocessFunction = (
  ...
  for (const detection of nmsDetections) {
    ...
    // Draw label with distance - white text with thick black outline
    ctx.font = 'bold 18px Arial';
    ctx.fillStyle = '#ffffff'; // white text
    ctx.strokeStyle = '#000000'; // black outline
    ctx.lineWidth = 5; // thicker outline for better visibility
    
    // Draw class label (top line)
    ctx.strokeText(label, x0 + 5, y0 - 30);
    ctx.fillText(label, x0 + 5, y0 - 30);
    
    // Draw distance (bottom line)
...
const postprocessYolov10: PostprocessFunction = (
  ...
  for (let i = 0; i < tensor.dims[1]; i += 6) {
    ...
    // Calculate bounding box dimensions for distance
    const bboxWidth = x1 - x0;
    const bboxHeight = y1 - y0;
